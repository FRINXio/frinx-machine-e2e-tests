...
describe("master test for checking multiple device inventory settings",()=>{
    ...
    beforeEach(()=>{
        cy.get("http://localhost/frinxui").as("mainPage")
        cy.get("http://localhost/frinxui/inventory/devices").as("deviceInventory")
    ...
    it("should find device by name and install afterwards",()=>{
        ...
        beforeEach(()=>{
            cy.get("span[class='chakra-checkbox__control css-xxkadm']").as("checkboxSquare")
            cy.get("button[class='chakra-button css-gjwknl']").as("settingButton")
        })
        ...
        cy.get("$checkboxSquare").eq(3).click()
        cy.get("$checkboxSquare").eq(4).click()
        cy.get("$checkboxSquare").eq(7).click()
        cy.contains("Install selected").click()
        cy.get("span[class='chakra-text css-1wihwec']",{timeout:80000}).should("be.visible")
        cy.get("$checkboxSquare").eq(3).should('have.attr', 'data-disabled')
        cy.get("$checkboxSquare").eq(4).should('have.attr', 'data-disabled')
        cy.get("$checkboxSquare").eq(7).should('have.attr', 'data-disabled')
        cy.get("$settingButton").eq(2).should("have.attr", "disabled")
        cy.get("$settingButton").eq(3).should("have.attr", "disabled")
        cy.get("$settingButton").eq(6).should("have.attr", "disabled")
